...
std::set<int> MainWindow::updateNeuronList()
{
  ...
  for (const auto &n : _scene->neurons())
  {
    ...
    if (_neuronAdditionalText->isChecked() && (function + layer > 0))
    {
      ...
      if (function > 0)
        functionText += QString::fromStdString(nsol::Neuron::functionToString(function));
      if (layer > 0)
        layerText += QString("layer ") + QString::number(static_cast<unsigned int>(layer));
      if (function > 0 && layer > 0)
        separator = ", ";
...
void MainWindow::loadCameraPositions()
{
  ...
  if (numActions > 0)
  {
    const auto warnText = tr("Loading new camera positions will remove"
                             " %1 existing position%2. Are you sure?")
                              .arg(
                                  numActions)
                              .arg(
                                  numActions > 1 ? "s" : "");
...
void MainWindow::onDataLoaded()
{
  ...
  auto hasMorphology = [](const std::pair<const unsigned int, nsol::NeuronPtr> &p)
  { return p.second->morphologicalType() != 0; };
  const bool hasMorphoData = std::find_if(_scene->neurons().cbegin(), _scene->neurons().cend(), hasMorphology) != _scene->neurons().cend();

  auto hasLayer = [](const std::pair<const unsigned int, nsol::NeuronPtr> &p)
  { return p.second->layer() != 0; };
  const bool hasLayerData = std::find_if(_scene->neurons().cbegin(), _scene->neurons().cend(), hasLayer) != _scene->neurons().cend();

  auto hasFunctionality = [](const std::pair<const unsigned int, nsol::NeuronPtr> &p)
  { return p.second->functionalType() != 0; };
